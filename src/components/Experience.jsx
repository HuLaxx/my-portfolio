                ...
                <div className="p-8 md:p-12">
                    ...
                    <div className="flex flex-col gap-6">
                        {experience?.map((job, index) => {
                            const CardContent = () => (
                                <div className="group relative h-full min-h-[320px] p-8 md:p-10 rounded-3xl border border-[var(--border)] bg-[var(--card)]/30 backdrop-blur-sm overflow-hidden transition-all duration-700 hover:shadow-2xl hover:translate-y-[-4px] hover:bg-[var(--card-hover-tint)] hover:border-[var(--accent)] flex flex-col justify-between">
                                    ...
                                    <div className="relative z-10 flex flex-col gap-6 h-full">
                                        <div className="flex justify-between items-start">
                                            <div className="flex flex-col gap-2">
                                                <span className="font-mono text-sm font-semibold uppercase tracking-[0.22em] text-[var(--accent-dark)] opacity-80 group-hover:text-[var(--text-hover)] transition-colors duration-500">
                                                    {job.period}
                                                </span>
                                                <h3 className="display-heading text-2xl font-semibold tracking-tight text-[var(--foreground)] md:text-3xl group-hover:text-[var(--text-hover)] transition-colors duration-500">
                                                    {job.role}
                                                </h3>
                                            </div>
                                    ...
                                    {/* Right-Side Hover Expansion - Only if clickable */}
                                    {job.slug && (
